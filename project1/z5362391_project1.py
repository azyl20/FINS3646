...
def get_tics(pth):
    ...
    ticker_file = open(pth, 'r')
    tickers = ticker_file.readlines()
    # extracts the ticker and exchange from each line in the text file
    for ticker in tickers:
        ticker_lower = ticker.split('"')[3].lower()
        exchange_lower = ticker.split('"')[1].lower()
        ticker_dict[ticker_lower] = exchange_lower
...
def read_dat(tic):
    ...
    tic_file = os.path.join(DATDIR, f'{tic}_prc.dat')

    lines_list = []
    ticker_file = open(tic_file, 'r')
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        line_dict[column] = line[index: index + COLWIDTHS[column]]
        index += COLWIDTHS[column]
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if tickers_lst == []:
            raise Exception('tickers_lst is not allowed to be an empty list')

        for ticker in tickers_lst:
            if ticker not in list(tic_exchange_dic.keys()):
                raise Exception("Ticker not in list")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if col_lst == []:
            raise Exception("col_list cannot be an empty list")

        for col in col_lst:
            if col not in COLUMNS:
                raise Exception("column not in COLUMNS")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    ticker_filter = []
    if tickers_lst is not None:
        ticker_filter = [ticker for ticker in list(tic_exchange_dic.keys()) if ticker not in tickers_lst]

    #print(tickers_lst)
    #print(ticker_filter)
    #print(list(tic_exchange_dic.keys()))
    '''
        If ticker list is none, we want all of the tickers, so ticker_filter will be an empty list.
        If this is the case, line 329 will always be true. 
        
        Otherwise, ticker_filter will contain the tickers for all the stocks other than the ones in ticker_lst.
        So only the tickers in ticker_list will execute the code within the if statement 
    '''
    for ticker in tic_exchange_dic.keys():
        if ticker not in ticker_filter:
            data_list = []
            ticker_dict = {}
            ticker_dict['exchange'] = tic_exchange_dic[ticker]
            dat_lines = read_dat(ticker)
            for line in dat_lines:
                line_dict = line_to_dict(line)
                if col_lst is not None:
                    # Using dictionary comprehension to filter the cols that we want if col_list is not None.
                    filtered_dict = {col: line_dict[col] for col in col_lst}
                    data_list.append(filtered_dict)
                else:
                    # Otherwise, take every col
                    data_list.append(line_to_dict(line))
            ticker_dict['data'] = data_list
            data[ticker] = ticker_dict
...
def _test_create_data_dict():
    ...
    for tic in list(tic_exchange_dic.keys()):
        data_dict[tic]['data'] = data_dict[tic]['data'][:3]
...
if __name__ == "__main__":
    ...
    # _test_get_tics()
    # _test_read_dat()
    # verify_tickers(get_tics(TICPATH), ['t', 'aapl', 'aal'])
    # _test_line_to_dict()
    _test_create_data_dict()
    # print(create_data_dict(get_tics(TICPATH), ['aapl', 'baba'], ['Date', 'Close']))
